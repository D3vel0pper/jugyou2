  ...
  */
  ...
  if($stmt->execute(array($login_id))) {
    if($row = $stmt->fetch()) {
      ...
      //if user is not exists, register user by login_id.
      $stmt = $pdo->prepare('INSERT INTO users (login_id) values (:login_id)');
      $stmt->bindParam(':login_id', $login_id);
      if($stmt->execute()) {
        $found = $pdo->lastInsertTd();
      }
      print("insert result: ${found}");
    }
  }
